    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (checkEmail) {
                    ...
                    if (cart != null) {
                        ...
                        if (!product.isEmpty()) {
                            for (ProductDTO pro : product.values()) {
                                totalOrderPrice += (pro.getProductPrice() * pro.getQuantityPurchased());
                                orderDetails.add(new OrderDetailDTO(0, pro.getProductPrice(), pro.getQuantityPurchased(), order.getOrderID(), pro.getProductID()));
                                int inventory = productDao.getProductQuantity(pro.getProductID());
                                boolean check = productDao.updateProductQuantity(pro.getProductID(), (inventory - pro.getQuantityPurchased()));
                                if (check == false) {
                                    break;
                                }
                            }
                ...
                UserDTO newUser = new UserDTO(email, "123456", 2, address, name, birthday, phone);
                boolean checkCreate = userDAO.createUser(newUser);
                if (checkCreate) {
                    OrderDAO orderDao = new OrderDAO();
                    HttpSession session = request.getSession();
                    if (session != null) {
